> **SUPERSEDED — REAL DATA, NOT FOR COMPARATIVE CLAIMS.** This dataset was
> measured with a systematically confounded run order (all mutex cells before
> all SPSC cells in every session) and an earlier retry/yield policy. The
> canonical Phase-2 dataset is `../spsc-throughput/`. See `SUPERSEDED.md` in
> this directory. The metadata below is retained as published at the time.

# Experiment 02, Phase 2 — throughput results (Apple M3 Max) — SUPERSEDED
