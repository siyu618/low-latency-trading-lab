...
WL=(A B C D E)
ROUNDS=5
# updates per workload at scale 1M. A and C stay at 2M so they reproduce the
# frozen Phase-2 reference flat_A_1M=4.327 / flat_C_1M=5.912 (harness
# cross-check). B uses fewer updates ONLY to bound wall time: workload B's
# off-clock STREAM GENERATION degenerates at near-full density (rnd_absent
# retries 4096 draws per add that almost never finds an absent level), ~31 s per
# 2M-update process, independent of the implementation and untimed. The timed
# apply loop is unaffected; B's flat-vs-bits delta is still measured on
# identical streams at identical updates.
for wl in "${WL[@]}"; do
