...
- `phase3-macos-apple-silicon/` — Phase 3M **Apple Instruments** observations
  (Time Profiler call trees, CPU Counters where supported) on the same Apple M3
  Max host that produced the Phase 2 dataset, read against that same-host
  ns/update. Present but EMPTY — no real Instruments recording yet; see
