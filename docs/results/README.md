...
- `phase2-m3max/` — the Phase 2 canonical **throughput** dataset: steady-state
  ...
  on the Apple M3 Max dev machine. Frozen. See its `RESULTS_METADATA.md`.
- `phase3-linux-<machine>/` — Phase 3 **Linux perf** counter data (one
  subdirectory per profiled cell), once a Linux host has actually run the
  harness (`scripts/perf-profile.sh`). Not yet present — Phase 3B is pending;
